...
### 重新编译 WPF 悬浮窗
```bash
node scripts/make-icon.mjs   # （可选）重新生成 app.ico 应用图标
node compile.mjs             # 调用 .NET Framework 4 csc.exe（C#5 语法兼容，自动嵌入 app.ico）
```
...
> 💡 **找不到窗口了？** 程序常驻任务栏和系统托盘（右下角图标）：最小化后点任务栏图标、或**双击托盘图标**即可找回悬浮窗；托盘右键 →「退出」可完全结束程序。
